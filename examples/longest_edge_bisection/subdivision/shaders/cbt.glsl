...
#endif
layout(set = 0, binding = CBT_HEAP_BUFFER_BINDING, std430)
 ...
 */
void
cbt__SetBitValue(const int cbtID, uint bufferID, uint bitID, uint bitValue)
{
 ...
 *
 */
cbt_Node cbt__CeilNode_Fast(const int cbtID, in const cbt_Node node)
{
    int maxDepth = cbt_MaxDepth(cbtID);
    return cbt_CreateNode(node.id << (maxDepth - node.depth), maxDepth);
}

cbt_Node cbt__CeilNode(const int cbtID, in const cbt_Node node)
{
    return cbt_IsNullNode(node) ? node : cbt__CeilNode_Fast(cbtID, node);
}
 ...
 *
 */
uint cbt__NodeBitID(const int cbtID, in const cbt_Node node)
{
 ...
 */
uint cbt__NodeBitID_BitField(const int cbtID, in const cbt_Node node)
{
    return cbt__NodeBitID(cbtID, cbt__CeilNode(cbtID, node));
}
 ...
 *
 */
int cbt__NodeBitSize(const int cbtID, in const cbt_Node node)
{
...
cbt__HeapArgs
cbt__CreateHeapArgs(const int cbtID, in const cbt_Node node, int bitCount)
{
    uint alignedBitOffset = cbt__NodeBitID(cbtID, node);
    uint maxHeapIndex = cbt__HeapUint32Size(cbt_MaxDepth(cbtID)) - 1u;
...
uint bitData
    ...
    cbt__HeapArgs args = cbt__CreateHeapArgs(cbtID, node, bitCount);

    cbt__BitFieldInsert(cbtID,
    args.heapIndexLSB,
    args.bitOffsetLSB,
    args.bitCountLSB,
    bitData);
...
void cbt__HeapWrite(const int cbtID, in const cbt_Node node, uint bitData)
{
    cbt__HeapWriteExplicit(cbtID, node, cbt__NodeBitSize(cbtID, node), bitData);
}
 ...
 */
uint
cbt__HeapReadExplicit(const int cbtID, in const cbt_Node node, int bitCount)
{
    cbt__HeapArgs args = cbt__CreateHeapArgs(cbtID, node, bitCount);
    uint lsb = cbt__BitFieldExtract(u_CbtBuffers[cbtID].heap[args.heapIndexLSB],
    args.bitOffsetLSB,
    args.bitCountLSB);
    uint msb = cbt__BitFieldExtract(u_CbtBuffers[cbtID].heap[args.heapIndexMSB],
    0u,
    args.bitCountMSB);
 ...
 */
void
cbt__HeapWrite_BitField(const int cbtID, in const cbt_Node node, uint bitValue)
{
    uint bitID = cbt__NodeBitID_BitField(cbtID, node);

    cbt__SetBitValue(cbtID, bitID >> 5u, bitID & 31u, bitValue);
}
 ...
 *
 */
uint cbt__HeapRead_BitField(const int cbtID, in const cbt_Node node)
{
    uint bitID = cbt__NodeBitID_BitField(cbtID, node);

    return cbt__GetBitValue(u_CbtBuffers[cbtID].heap[bitID >> 5u], bitID & 31u);
}
