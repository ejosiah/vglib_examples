...
float sampleVolume(Ray ray, Bounds bounds, out float t, out vec3 samplePos){
    const float scale = ray.tFar - ray.tNear;
    const float tMax = ray.tFar;
    t = ray.tNear;
    while(t < tMax){
        t -= log(1 - rand(rngState)) * scale * vd.invMaxDensity;
        if(t >= tMax) {
            break;
        }
        vec3 pos = ray.origin + ray.direction * t;
        samplePos = uvw(pos, bounds);
        float voxel = texture(volume, samplePos).r;
        if(voxel * vd.invMaxDensity > rand(rngState)){
            return voxel;
        }
    }
    return 0;
...
vec4 ray_march(Ray ray, Bounds bounds){
    ...
    for(int i = 0; i < vd.numSamples; i++){
        ...
        vec3 samplePos;
        float t;
        float voxel = sampleVolume(ray, bounds, t, samplePos);
