...
vec3 reconstructViewPos(vec2 uv, float depth) {
    ...
    vec4 viewPos = ubo.inverseProjection * ndc;
    return viewPos.xyz / viewPos.w;
}

void main() {
    vec2 texel = 1.0 / ubo.viewportSize;
    ...
    vec4 viewPos = ubo.inverseProjection * currentNdc;
    viewPos.xyz /= viewPos.w;
    vec4 currentWorldPos = ubo.inverseView * vec4(viewPos.xyz, 1);

    vec4 previousNdc = ubo.previousViewProjection * currentWorldPos;
    ...
    float ldepth = length(viewPos)/(ubo.far - ubo.near);
