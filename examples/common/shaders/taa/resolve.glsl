...
layout(constant_id = 0) const uint history_filter_type = HISTORY_SAMPLING_FILTER_CATMULL_ROM;
layout(constant_id = 1) const uint subsample_filter_type = SUB_SAMPLE_FILTER_MICHELL;
layout(constant_id = 2) const uint history_constraint_type = HISTORY_CONSTRIANT_CLAMP;
layout(constant_id = 3) const uint temporal_filtering = 1;
layout(constant_id = 4) const uint inverse_luminance_filtering = 1;
layout(constant_id = 5) const uint luminance_difference_filtering = 1;
layout(constant_id = 6) const uint taa_simple = 0;
