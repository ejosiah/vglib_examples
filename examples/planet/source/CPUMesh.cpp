...
void fill_bisectors(cc_Mesh* targetMesh, uint32_t cbtNumElements,
    std::vector<uint64_t>& heapIDArray, std::vector<glm::uvec3>& neighborsArray,
    std::vector<glm::vec3>& basePoints, uint32_t& totalNumElements, uint32_t& minimalDepth)
{
    ...
    memset(neighborsArray.data(), 0, totalNumElements * sizeof(glm::uvec3));
    ...
    for (uint32_t halfEdgeIdx = 0; halfEdgeIdx < (uint32_t)targetMesh->halfedgeCount; ++halfEdgeIdx)
    {
        ...
        basePoints[3 * halfEdgeIdx + 2] = glm::vec3({ p2.x, p2.y, p2.z });
        ...
        basePoints[3 * halfEdgeIdx + 0] = glm::vec3({ p0.x, p0.y, p0.z });

        // Let's compute the third vertex
        glm::vec3 thirdVertex = basePoints[3 * halfEdgeIdx + 2];
        ...
        basePoints[3 * halfEdgeIdx + 1] = glm::vec3({ thirdVertex.x / sumWeight, thirdVertex.y / sumWeight, thirdVertex.z / sumWeight });
    }
}
}

CPUMesh CPUMesh::load_cpu_mesh(const std::filesystem::path &meshPath, uint32_t cbtNumElements) {
    CPUMesh outputMesh{};

    cc_Mesh* targetMesh = ccm_Load(meshPath.string().data());
