...
const uint64_t one64_t = 1;
int bitCount64(uint64_t v) { return bitCount(uint(v)) + bitCount(uint(v >> 32ul)); }
...
void reduce_prepass(uint dispatchThreadID)
{
    ...
    for (uint pairIdx = 0; pairIdx < 4; ++pairIdx)
    {
        ...
        uint elementC = uint(bitCount64(_BitfieldBufferRW[dispatchThreadID * 8 + 2 * pairIdx]));

        // Second element of the pair
        elementC += uint(bitCount64(_BitfieldBufferRW[dispatchThreadID * 8 + 2 * pairIdx + 1]));
