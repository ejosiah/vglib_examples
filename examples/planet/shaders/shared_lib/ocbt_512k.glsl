#ifndef OCBT_512K_GLSL
#define OCBT_512K_GLSL
...
Level 3: 32 bit // [0, 65536] x 8, needs a minimum of 17 bits (rounded up to 32 for alignment and required for atomic operations)
Level 4: 32 bit // [0, 32768] x 16, needs a minimum of 16 bits (bumped to 32 bits for atomic operations)
Level 5: 32 bit // [0, 16384] x 32, needs a minimum of 15 bits (rounded up to 16 for alignment and bumped to 32 bits for atomic operations)
...
Level 9: 16 bit // [0, 1024] x 512, needs a minimum of 11 bits (rounded up to 16 for alignment)
Level 10: 16 bit // [0, 512] x 1024, needs a minimum of 10 bits (rounded up to 16 for alignment)
Level 11: 16 bit // [0, 256] x 2048, needs a minimum of 9 bits (rounded up to 16 for alignment)
Level 12: 8 bit // [0, 128] x 4096, needs a minimum of 8 bits

Level 13: Raw 64 bits representation
*/

// Num elements
#define OCBT_NUM_ELEMENTS 524288u
// Tree sizes
#define OCBT_TREE_SIZE_BITS (32u + 32u * 2u + 32u * 4u + 32u * 8u + 32u * 16u + 32u * 32u + 32u * 64u + 16u * 128u + 16u * 256u + 16u * 512u + 16u * 1024u + 16u * 2048u + 8u * 4096u)
#define OCBT_TREE_NUM_SLOTS (OCBT_TREE_SIZE_BITS / 32u)
#define OCBT_BITFIELD_NUM_SLOTS (OCBT_NUM_ELEMENTS / 64u)
#define OCBT_LAST_LEVEL_SIZE 4096u

// Tree last level
#define TREE_LAST_LEVEL 12u
// First virtual level
#define FIRST_VIRTUAL_LEVEL 13u
// Leaf level
#define LEAF_LEVEL 19u

// per level offset
const uint OCBT_depth_offset[20] = uint[20](
    0u, // Level 0
    32u * 1u, // level 1
...
const uint64_t OCBT_bit_mask[20] = uint64_t[20](
...
const uint OCBT_bit_count[20] = uint[20](
    32u, // Root 17
    32u, // Level 16
    32u, // level 15
    32u, // level 14
...
void reduce_first_pass(uint dispatchThreadID, uint groupIndex)
{
    ...
    _CBTBufferRW[level3Offset + dispatchThreadID] = gs_cbtTree[level3Offset + dispatchThreadID];

    const uint level4Offset = OCBT_depth_offset[TREE_LAST_LEVEL - 3u] / 32u;
    if (groupIndex % 2u == 0u)
        _CBTBufferRW[level4Offset + dispatchThreadID / 2u] = gs_cbtTree[level4Offset + dispatchThreadID / 2u];
}
#endif // CAN_USE_SHARED_MEMORY

#endif // OCBT_512K_GLSL
