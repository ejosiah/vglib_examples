...
struct Geometry {
    ...
    void initialize(VulkanDevice& device, FileManager& fileManager) {
        ...
        for(auto& v : c.vertices) {
            v.position = glm::vec4(v.position.xyz() * 2.f, 1);
        }
        ...
        mesh::load(meshes, fileManager.getFullPath("cow.ply")->string());
