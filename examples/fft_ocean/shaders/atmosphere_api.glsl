#ifndef ATMOSPHERE_ALL
#define ATMOSPHERE_ALL
...
Length ComputeOpticalLengthToTopAtmosphereBoundary(AtmosphereParameters atmosphere, DensityProfile profile, Length r,
                                                   Number mu) {
...
ComputeTransmittanceToTopAtmosphereBoundary(AtmosphereParameters atmosphere, Length r, Number mu) {
    assert(r >= atmosphere.bottom_radius && r <= atmosphere.top_radius);
    assert(mu >= -1.0 && mu <= 1.0);
    return exp(-(
...
void GetRMuFromTransmittanceTextureUv(AtmosphereParameters atmosphere, vec2 uv, out Length r, out Number mu) {
...
DimensionlessSpectrum
ComputeTransmittanceToTopAtmosphereBoundaryTexture(AtmosphereParameters atmosphere, vec2 frag_coord) {
...
DimensionlessSpectrum
GetTransmittanceToTopAtmosphereBoundary(AtmosphereParameters atmosphere, TransmittanceTexture transmittance_texture,
                                        Length r, Number mu) {
    assert(r >= atmosphere.bottom_radius && r <= atmosphere.top_radius);
...
DimensionlessSpectrum
GetTransmittance(AtmosphereParameters atmosphere, TransmittanceTexture transmittance_texture, Length r, Number mu,
...
#define IRRADIANCE_TEXTURE global_textures[IRRADIANCE_TEXTURE_ID]
#define TRANSMITTANCE_TEXTURE global_textures[TRANSMITTANCE_TEXTURE_ID]
#define SCATTERING_TEXTURE global_textures3d[SCATTERING_TEXTURE_ID]
#define SIGNLE_MIE_SCATTERING_TEXTURE global_textures3d[SCATTERING_TEXTURE_ID]
...
Illuminance3
GetSunAndSkyIlluminance(Position p, Direction normal, Direction sun_direction, out IrradianceSpectrum sky_irradiance) {
    p /= ubo.lengthUnitInMeters;
