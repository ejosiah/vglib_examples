#ifndef PATH_TRACING_BRDF_GLSL
#define PATH_TRACING_BRDF_GLSL
...
vec3 sampleSpecular(int specular_brdf_type, vec3 Vlocal, float alpha, float alphaSquared, vec3 specularF0, vec2 u,out vec3 weight){
...
vec3 evalIndirectCombinedBRDF(inout BrdfArgs args){
    ...
    if((args.brdfType & BRDF_DIFFUSE) != 0){
    ...
    }else if((args.brdfType & BRDF_SPECULAR) != 0){
        localArgs.wi = vec3(0, 0, 1);   // not used for specular
        BrdfData data = prepareBRDFData(localArgs);
        vec2 u = randomVec2(args.rngState);
        localArgs.wi = sampleSpecular(args.brdfType, localArgs.wo, data.alpha, data.alphaSquared, data.specularF0, u, sampleWeight);
    }
