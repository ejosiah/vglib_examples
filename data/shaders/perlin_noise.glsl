...
float perlin_fbm(vec2 p, float freq, int octaves, float persistence, float lacunarity) {
    float total = 0.0;
    float amplitude = 1.0;
    float frequency = freq;

    for (int i = 0; i < octaves; i++) {
        total += amplitude * perlin(p * frequency);
...
float perlin_fbm(vec2 p, float freq, int octaves) {
    return perlin_fbm(p, freq, octaves, 0.5, 2.0);
}


float perlin_fbm(vec3 p, float freq, int octaves, float persistence, float lacunarity) {
    float total = 0.0;
    float amplitude = 1.0;
    float frequency = freq;
